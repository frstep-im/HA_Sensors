...
The authenticated event explorer defaults to the last 48 hours and gives each Home Assistant entity and Soter event type its own timeline lane. It displays meaningful movement, positive appliance use, TV viewing periods, occupant leaving, occupant returning, visitor arrival, door opening, and door-left-open events on a local-time axis; Soter event meanings retain fixed colours across ranges and filters. A rolling one-hour activity graph shows total movement and each sensor as stepped lines. Charts use three-hour local-time grid lines, show details on hover, and support drag-to-select time zoom. Raw activity lists are intentionally omitted.
