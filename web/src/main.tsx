...
interface Config { enabled: boolean; householdId: string; homeAssistantBaseUrl: string; timezone: string; entities: Entity[]; windowMinutes: number; baselineDays: number; minimumBaselineWindows: number; alertThreshold: number; consecutiveWindows: number; webhookEnabled: boolean; lastCollectedAt: string | null; soterProjectId: string; soterDeviceId: string }
...
interface Overview { config: Config; health: { lastCollectionStatus?: string; lastCollectionError?: string; lastCollectedAt?: string }; windows: Window[]; alerts: Alert[] }
...
function Dashboard({ user, previewData }: { user: User; previewData?: Overview }) {
  ...
  return <div>
    ...
    <main id="top">{error && <div className="banner"><CircleAlert />{error}<button onClick={() => setError("")}>Dismiss</button></div>}
      <section className="hero" id="overview"><div><p className="eyebrow">68 Grenehurst Way · household live view</p><h1>{headline(latest?.status)}</h1><p className="hero-copy">{latest?.reasons?.[0] ?? "Connect Home Assistant to begin learning this household’s routine."}</p><div className="status-line"><i className={data.config.enabled ? "online" : "paused"} />{data.config.enabled ? "Monitoring" : "Collection paused"}<span />Last update {relative(data.health.lastCollectedAt ?? data.config.lastCollectedAt)}</div></div><GaugeView value={latest?.normalityIndex ?? null} status={latest?.status} /></section>
...
function SettingsDrawer({ config, busy, close, save, backfill }: { config: Config; busy: string; close: () => void; save: (c: Config) => void; backfill: () => void }) {
  ...
  return <div className="backdrop" onMouseDown={(e) => e.target === e.currentTarget && close()}><aside className="drawer"><header><div><p className="eyebrow">Household controls</p><h2>Data connection</h2></div><button className="icon" onClick={close}><X /></button></header><div className="drawer-body"><label className="switch"><span><strong>Scheduled collection</strong><small>Pull and analyse every five minutes</small></span><input type="checkbox" checked={draft.enabled} onChange={(e) => update({ enabled: e.target.checked })} /></label><label>Home Assistant HTTPS URL<input value={draft.homeAssistantBaseUrl} placeholder="https://example.ui.nabu.casa" onChange={(e) => update({ homeAssistantBaseUrl: e.target.value })} /></label><label>Timezone<input value={draft.timezone} onChange={(e) => update({ timezone: e.target.value })} /></label><label>Entities <small>one per line: entity_id, kind, label</small><textarea rows={7} value={entities} placeholder={"binary_sensor.hall_motion,motion,Hall\nsensor.kettle_power,power,Kettle"} onChange={(e) => setEntities(e.target.value)} /></label><div className="field-pair"><label>Alert below<input type="number" value={draft.alertThreshold} onChange={(e) => update({ alertThreshold: Number(e.target.value) })} /></label><label>Repeated windows<input type="number" value={draft.consecutiveWindows} onChange={(e) => update({ consecutiveWindows: Number(e.target.value) })} /></label></div><div className="field-pair"><label>Baseline days<input type="number" value={draft.baselineDays} onChange={(e) => update({ baselineDays: Number(e.target.value) })} /></label><label>Minimum samples<input type="number" value={draft.minimumBaselineWindows} onChange={(e) => update({ minimumBaselineWindows: Number(e.target.value) })} /></label></div><label className="switch"><span><strong>Alert webhook</strong><small>Send JSON only after the threshold persists</small></span><input type="checkbox" checked={draft.webhookEnabled} onChange={(e) => update({ webhookEnabled: e.target.checked })} /></label><div className="secret"><ShieldCheck /><div><strong>Token stored outside the app</strong><p>The Home Assistant token stays in Google Secret Manager.</p><code>npm run secret:ha</code></div></div><div className="backfill"><div><strong>Build the initial baseline</strong><small>Run after saving the URL, entities and token.</small></div><button className="secondary" disabled={!!busy} onClick={backfill}>{busy === "backfill" ? "Working…" : "Backfill 14 days"}</button></div></div><footer><button className="secondary" onClick={close}>Cancel</button><button className="primary compact" disabled={!!busy} onClick={() => save({ ...draft, entities: parsed })}>{busy === "save" ? "Saving…" : "Save settings"}</button></footer></aside></div>;
}
